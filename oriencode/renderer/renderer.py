...
class Renderer2D_Rotation(Renderer2D):
    def __init__(self, px_size, sigma_blur, rotation_angle=0, **kwargs):
        """
        Extended 2D renderer with support for rotating the extent box.

        Args:
            rotation_angle: angle in degrees to rotate the rectangle.
            px_size: pixel size of the output image in nm
            sigma_blur: sigma of the gaussian blur applied in nm
            Other arguments inherited from Renderer2D...
        """
        super().__init__(px_size, sigma_blur, **kwargs)
        self.rotation_angle = rotation_angle

    def render(self, em, col_vec=None, ax=None):
        if ax is None:
            ax = plt.gca()

        # Rotate the extent along with the histogram rendering
        hist = self.forward(em, col_vec).numpy()
        rotated_hist = self.rotate_image(hist)

        if col_vec is not None:
            if self.colextent:
                ax.imshow(np.transpose(rotated_hist, [1, 0, 2]))
            else:
                ax.imshow(np.transpose(rotated_hist), cmap='gray')
        else:
            ax.imshow(np.transpose(rotated_hist), cmap='gray')
    ...
    def rotate_image(self, image):
        ...
        indices = np.indices((h, w)).reshape(2, -1)
        indices_centered = indices - np.array([[center_y], [center_x]])

        rotated_indices = rotation_matrix @ indices_centered
        rotated_indices += np.array([[center_y], [center_x]])

        # Interpolating/reference rotated positions
        rotated_image = np.zeros_like(image)
        for i in range(h):
            for j in range(w):
                y, x = rotated_indices[:, i * w + j]
