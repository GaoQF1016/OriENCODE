...
class AttentionBranch(nn.Module):
    ...
    def __init__(self, nf, k_size=3):
        ...
        self.fusion_conv = nn.Conv2d(nf*2, nf, kernel_size=k_size, padding=(k_size - 1) // 2, bias=False)
    ...
    def forward(self, x):
        ...
        y = torch.cat([x_branch, y_branch], dim=1)
        y = self.sigmoid(self.fusion_conv(y))
...
class AdaptiveSwinTransformerBlock(nn.Module):
    def __init__(self, embed_dim=192, num_heads=4, window_size=8, mlp_ratio=4., qkv_bias=True, attn_drop=0., drop_path=0.):
        super().__init__()
        self.window_size = window_size
        self.embed_dim = embed_dim

        dummy_resolution = (32, 32)  

        self.block = SwinTransformerBlock(
            dim=embed_dim,
            input_resolution=dummy_resolution,
            num_heads=num_heads,
    ...
    def forward(self, x):
        ...
        # 动态更新 input_resolution
        self.block.input_resolution = (H, W)
...
class UNetBase(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, depth=4, initial_features=64, gain=2, pad_convs=False, norm=None,
        ...
        self.swin = AdaptiveSwinTransformerBlock(
                                                        embed_dim=192,
                                                        num_heads=4,
                                                        window_size=8,
                                                        mlp_ratio=4.0,
...
class model9(DoubleMUnet):
    ...
    def __init__(self, ch_in: int, *, depth_shared: int, depth_union: int, initial_features: int, inter_features: int,
        ...
        self.mt_heads = torch.nn.ModuleList(
            [MLTHeads(in_channels=inter_features, out_channels=ch_out,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        feature = self._forward_core(x)

        """Forward through the respective heads"""
        x_heads = [mt_head.forward(feature) for mt_head in self.mt_heads]
...
class Discriminator(nn.Module):
    def __init__(self, in_channels=48):
        ...
        self.model = nn.Sequential(
            nn.Conv2d(48, 16, kernel_size=4, stride=2, padding=1),  
            nn.LeakyReLU(0.2),
            nn.Conv2d(16, 32, kernel_size=4, stride=2, padding=1),  
            nn.LeakyReLU(0.2),
            nn.Conv2d(32, 1, kernel_size=4, stride=4, padding=0), 
            nn.Flatten(),  
            nn.Linear(4, 1), 
            nn.Sigmoid()  
        )
