...
class AttentionBranch(nn.Module):
    ...
    def forward(self, x):
        ...
        grad_x = self.lrelu(self.x_conv(x))
        grad_y = self.lrelu(self.y_conv(x))
        grad_magnitude = torch.sqrt(grad_x**2 + grad_y**2 + 1e-8)
        norm_grad_x = grad_x / grad_magnitude
        norm_grad_y = grad_y / grad_magnitude
        div_x = self.x_conv(norm_grad_x)
        div_y = self.y_conv(norm_grad_y)
        kappa = div_x + div_y
        weight = torch.sigmoid(kappa)
        out = torch.mul(self.k3(x), weight)
        out = self.k4(out)
...
class UNetBase(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, depth=4, initial_features=64, gain=2, pad_convs=False, norm=None,
        ...
        self.encoder = nn.ModuleList([nn.Sequential(
                                self._conv_block(n_features[level], n_features[level + 1], level, part='encoder', activation=activation),
                                SEBlock(n_features[level + 1]))
                                for level in range(self.depth)])
