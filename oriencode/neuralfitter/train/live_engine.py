...
def live_engine_setup(param_file: str, device_overwrite: str = None, debug: bool = False,
    ...
    ds_train, ds_test, model, model_ls, generator, generator_ls, discriminator, optimizer_g, optimizer_d, criterion, bceloss, lr_scheduler_g, lr_scheduler_d, grad_mod, post_processor, matcher, ckpt = \
        setup_trainer(sim_train, sim_test, psf, logger, model_out, ckpt_path, device, param)
    ...
    if from_ckpt:
        ...
        lr_scheduler_g.load_state_dict(ckpt.lr_sched_g_state)
        lr_scheduler_d.load_state_dict(ckpt.lr_sched_d_state)
    ...
    while not converges and n < n_max:
        ...
        for i in range(first_epoch, param.HyperParameter.epochs):
            logger.add_scalar('learning/learning_rate', optimizer_g.param_groups[0]['lr'], i)
            ...
            if i >= 1:
                if isinstance(lr_scheduler_g, torch.optim.lr_scheduler.ReduceLROnPlateau):
                    lr_scheduler_g.step(val_loss)
                else:
                    lr_scheduler_g.step()
            generator_ls.save(generator, None)
            if no_log:
                ckpt.dump(generator.state_dict(), optimizer_g.state_dict(), lr_scheduler_g.state_dict(),
                          step=i)
            else:
                ckpt.dump(generator.state_dict(), optimizer_g.state_dict(), lr_scheduler_g.state_dict(),
...
def setup_trainer(simulator_train, simulator_test, psf, logger, model_out, ckpt_path, device, param):
    ...
    models_available = {
        ...
        'New_model' : decode.neuralfitter.models.model_attention.New_model,
        'New_model_2' : decode.neuralfitter.models.model_attention_2.New_model_2,
        'New_model_3' : decode.neuralfitter.models.model_attention_3.New_model_3,
        'New_model_4' : decode.neuralfitter.models.model_attention_4.New_model_4,
        'New_model_5' : decode.neuralfitter.models.model_attention_5.New_model_5,
        'New_model_6' : decode.neuralfitter.models.model_attention_6.New_model_6,
        'New_model_7' : decode.neuralfitter.models.model_attention_7.New_model_7,
        'model_1' : decode.neuralfitter.models.model_1.model1,
        'model_2' : decode.neuralfitter.models.model_2.model2,
        'model_3' : decode.neuralfitter.models.model_3.model3,
        'model_4' : decode.neuralfitter.models.model_4.model4,
        'model_5' : decode.neuralfitter.models.model_5.model5,
        'model_6' : decode.neuralfitter.models.model_6.model6,
        'model_7' : decode.neuralfitter.models.model_7.model7,
        'model_8' : decode.neuralfitter.models.model_8.model8,
        'model_9' : decode.neuralfitter.models.model_9.model9

    }
    input_file = '/home/GaoQF/Downloads_original/2024-04-27_12-48-54_decode1channelin/model_0.pt'
    ...
    generator = decode.neuralfitter.models.model_1.Generator.parse(param)
    generator_ls = decode.utils.model_io.LoadSaveModel(generator,
                                                   output_file=model_out,
                                                   input_file=input_file)
    generator = generator_ls.load_init()
    generator = generator.to(torch.device(device))
    '''for param in generator.parameters():
        param.requires_grad = True'''
        
    discriminator = decode.neuralfitter.models.model_1.Discriminator()
    discriminator = discriminator.to(torch.device(device))
    ...
    # 为生成器定义优化器 
    optimizer_g = optimizer_available[param.HyperParameter.optimizer]
    optimizer_g = optimizer_g(generator.parameters(), **param.HyperParameter.opt_param)
    '''optimizer_g = optimizer_available[param.HyperParameter.optimizer](
        generator.parameters(), **param.HyperParameter.opt_param)'''

    # 为判别器定义优化器
    optimizer_d = optimizer_available[param.HyperParameter.optimizer](
    ...
    criterion = decode.neuralfitter.loss.GaussianMMLoss(
        ...
        extra_weight=param.HyperParameter.extra_weight)
    ...
    lr_scheduler_g = lr_scheduler_available[param.HyperParameter.learning_rate_scheduler](
       optimizer_g, **param.HyperParameter.learning_rate_scheduler_param)

    lr_scheduler_d = lr_scheduler_available[param.HyperParameter.learning_rate_scheduler](
       optimizer_d, **param.HyperParameter.learning_rate_scheduler_param)
    ...
    return train_ds, test_ds, model, model_ls, generator, generator_ls, discriminator, optimizer_g, optimizer_d, criterion, bceloss, lr_scheduler_g, lr_scheduler_d, grad_mod, post_processor, matcher, checkpoint
