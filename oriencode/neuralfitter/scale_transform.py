...
class ParameterListRescale:
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor, bg: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:

        if x.dim() not in (2, 3) or x.size(-1) != 6:
        ...
        x[..., 3] = x[..., 3] / self.z_max
        bg = bg / self.bg_max
        #crlb_tar = crlb_tar.clone()
        #crlb_tar[..., 2] = crlb_tar[..., 2] / self.z_max
        #crlb_tar[..., 3] = crlb_tar[..., 3] / self.phot_max
