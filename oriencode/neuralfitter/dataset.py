...
class SMLMDataset(Dataset):
    ...
    def __init__(self, *, em_proc, frame_proc, bg_frame_proc, tar_gen, weight_gen,
        ...
        self._frames_em = None
        self._frames = None
        self._emitter = None
        self._primal_em = None
        self.psf = None
    ...
    def _process_sample(self, frames_em, frames, tar_emitter, bg_frame): #psf

        """Process"""
        if self.frame_proc is not None:
            frames = self.frame_proc.forward(frames)
            frames_em = self.frame_proc.forward(frames_em)
        ...
        return frames_em, frames, target, weight, tar_emitter

    def _return_sample(self, frames_em, frame, target, weight, emitter):

        if self.return_em:
            return frames_em, frame, target, weight, emitter
        else:
            return frames_em, frame, target, weight
...
class SMLMStaticDataset(SMLMDataset):
    ...
    def __init__(self, *, frames_em, frames, emitter: (None, list, tuple),
    ...
    def __getitem__(self, ix):
        ...
        frames_em = self._get_frames(self._frames_em, ix)
        frames = self._get_frames(self._frames, ix)
        bg_frame = self._bg_frames[ix] if self._bg_frames is not None else None

        frames_em, frames, target, weight, tar_emitter = self._process_sample(frames_em, frames, tar_emitter, bg_frame)

        return self._return_sample(frames_em, frames, target, weight, tar_emitter)
...
class InferenceDataset(SMLMStaticDataset):
    ...
    def __init__(self, *, frames_em, frames, frame_proc, frame_window):
        """
        ...
        """
        super().__init__(frames_em=frames_em, frames=frames, emitter=None, frame_proc=frame_proc, bg_frame_proc=None, em_proc=None,
                         tar_gen=None, pad='same', frame_window=frame_window, return_em=False)

    def _return_sample(self, frames_em, frame, target, weight, emitter):
        return frames_em, frame
...
class SMLMLiveDataset(SMLMStaticDataset):
    ...
    def __init__(self, *, simulator, em_proc, frame_proc, bg_frame_proc, tar_gen, weight_gen, frame_window, pad,
        ...
        super().__init__(emitter=None, frames_em=None, frames=None,
    ...
    def sample(self, verbose: bool = False):
        ...
        """Sample new dataset."""
        t0 = time.time()
        emm, frames_em, frames, bg_frames = self.simulator.sample()  #psf
        if verbose:
            print(f"Sampled dataset in {time.time() - t0:.2f}s. {len(emm)} emitters on {frames.size(0)} frames.")

        """Split Emitters into list of emitters (per frame) and set frame_ix to 0."""
        emitter = emm.split_in_frames(0, frames.size(0) - 1)
        emitter = [set_frame_ix(em) for em in emitter]

        self._emitter = emitter
        self._frames_em = frames_em.cpu()
...
class SMLMAPrioriDataset(SMLMLiveDataset):
    ...
    def sample(self, verbose: bool = False):
        ...
        """
        t0 = time.time()
        emitter, frames_em, frames, bg_frames = self.simulator.sample()

        if verbose:
            print(f"Sampled dataset in {time.time() - t0:.2f}s. {len(emitter)} emitters on {frames.size(0)} frames.")

        frames_em, frames, target, weight, tar_emitter = self._process_sample(frames_em, frames, emitter, bg_frames) #psf
        self._frames_em = frames_em.cpu()
        self._frames = frames.cpu()
    ...
    def __getitem__(self, ix):
        ...
        return self._return_sample(self._get_frames(self._frames_em, ix), self._get_frames(self._frames, ix), [tar[ix] for tar in self._target],  # target is tuple
...
class SMLMLiveSampleDataset(SMLMDataset):
    ...
    def __getitem__(self, ix):
        ...
        frames_em = self._get_frames(frames_em, (frames_em.size(0) - 1) // 2)
        ...
        frames_em, frames, target, weight, tar_emitter = self._process_sample(frames_em, frames, tar_emitter, bg_frames)
