...
class ParameterListTarget(TargetGenerator):
    def __init__(self, psf, n_max: int, xextent: tuple, yextent: tuple, ix_low=None, ix_high=None, xy_unit: str = 'px',
                 squeeze_batch_dim: bool = False):
        """
    ...
    def remove_duplicate_rows(self, xyz, phot, frame_ix, id):
        
        n_frames = frame_ix.max() - frame_ix.min() + 1
        xyz_list = []
        phot_list = []
        frame_ix_list = []
        id_list = []
        for i in range(frame_ix.min(), frame_ix.max() + 1):
            ix = frame_ix == i
            rounded_tensor = torch.floor(xyz[ix][:, :2])
            rounded_tensor_np = np.float64(rounded_tensor.numpy())
            _, unique_indices = np.unique(rounded_tensor_np, axis=0, return_index=True)
            unique_indices = np.sort(unique_indices)
            unique_xyz = xyz[ix][unique_indices]
            unique_phot = phot[ix][unique_indices]
            unique_frame_ix = frame_ix[ix][unique_indices]
            unique_id = id[ix][unique_indices]
            xyz_list.append(unique_xyz)
            phot_list.append(unique_phot)
            frame_ix_list.append(unique_frame_ix)
            id_list.append(unique_id)
        xyz = torch.cat(xyz_list, dim=0)
        phot = torch.cat(phot_list, dim=0)
        frame_ix = torch.cat(frame_ix_list, dim=0)
        id = torch.cat(id_list, dim=0)

        return xyz, phot, frame_ix, id

    def forward(self, em: EmitterSet, bg, ix_low: int = None, ix_high: int = None):  #psf
        em, ix_low, ix_high = self._filter_forward(em, ix_low, ix_high)

        n_frames = ix_high - ix_low + 1
        #locs_tar = torch.zeros((n_frames, 32, 32))
            
        """Setup and compute parameter target (i.e. a matrix / tensor in which all params are concatenated)."""
        param_tar = torch.zeros((n_frames, self.n_max, 6))
        ...
        for i in range(n_frames):
            ...
            ix = em.frame_ix == i
            #print(xyz[ix].shape, em[ix].phot.shape, bg[ix].shape)
            crlb, _ = self.psf.crlb(xyz[ix], em[ix].phot, bg[i])
            #crlb = np.sqrt(crlb)
            param_tar[i, :n_emitter, 0] = em[ix].phot
            param_tar[i, :n_emitter, 1:4] = xyz[ix]
            
            param_tar[i, :n_emitter, 4:] = crlb[:, :2]
            #locs_tar[i, torch.floor(em.xyz_px[ix][:, 0]).long(), torch.floor(em.xyz_px[ix][:, 1]).long()] = 1
        #em_xyz, em_phot, _, _ = self.remove_duplicate_rows(em.xyz_px, em.phot, em.frame_ix, em.id)
        #print(em_xyz, em_phot, bg)
        '''crlb, _ = self.psf.crlb(em_xyz, em_phot, bg)
        ...
        #print(locs_tar.shape)
        #print(crlb_tar.shape)
        
        return self._postprocess_output(param_tar), self._postprocess_output(mask_tar), bg#, crlb_tar, locs_tar
